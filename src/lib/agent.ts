...
export async function runAgent(opts: RunAgentOptions): Promise<void> {
  ...
  // Server-side tools (web_search_20260209 / web_fetch_20260209) run code
  // execution under the hood and produce a container that must be reused in
  // subsequent requests of the same agent turn. See `shared/tool-use-concepts`
  // in the claude-api skill: "container_id is required when there are pending
  // tool uses generated by code execution with tools."
  let containerId: string | null = null;
  // Accumulate everything the agent emits across iterations so we can
  // persist it as the run's summary at the end. Without this, the
  // agent_runs.summary column stays null and the dashboard shows
  // "(no summary)" for every run.
  ...
  try {
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
      ...
      // pause_turn — server-side tool loop hit its internal limit. Append the
      // assistant turn and re-send to continue. No custom tool execution
      // needed; the API resumes where it left off.
      ...
      for (const tool of toolUseBlocks) {
        if (tool.name === "propose_trade") proposalsCreated += 1;
        if (tool.name === "save_insight") insightsSaved += 1;
        await onEvent({
          type: "tool_use",
          id: tool.id,
          name: tool.name,
    ...
    // Build a summary that's never just blank. Prefer the agent's actual
    // text output; if the turn ended silently but something was produced,
    // fall back to the action counts; if absolutely nothing happened, say so.
    const textSummary = allText.join("").trim();
    const actionBits: string[] = [];
    if (proposalsCreated > 0)
      actionBits.push(`${proposalsCreated} proposals`);
